...
public class Deck {
	...
	public Deck(String[] ranks, String[] suits, int[] values) {
		...
		for (int i = 0; i < ranks.length; i++) {
			for (int j = 0; j < suits.length; j++) {
				Card makecard = new Card(ranks[i], suits[j], values[i]);
				cards.add(cardmaking, makecard);
				cardmaking++;
			}
		}
		size = cards.size();
	...
	public void shuffle() {
		...
		for (int k = cards.size() - 1; k >= 0; k--) {
			...
			Card tempr = cards.get(r);
			Card tempk = cards.get(k);
			int deathr = cards.indexOf(tempr);
			int deathk = cards.indexOf(tempk);
			cards.set(deathk, tempr);
			cards.set(deathr, tempk);
		}
		size = cards.size();
	}
